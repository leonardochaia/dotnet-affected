...
The site is not versioned: it documents whatever is on `main`, and publishes from it. There is no version picker and
no per-version content, so anything the site says has to be true of the newest release. Users on an older major are
served by the upgrade guide, and by the README at that major's tag.

### At every release

MSBuild SDK versions have to be exact — NuGet-based SDK resolution takes no floating ranges — so they are written out
in full and need bumping:

| File | What |
|---|---|
| `docs/src/content/docs/msbuild-sdk/index.md` | `Sdk="DotnetAffected.Tasks/…"` and the `global.json` pin |
| `docs/src/content/docs/msbuild-sdk/reference.md` | Two `DotnetAffected.Tasks` `PackageVersion` lines |
| `src/DotnetAffected.Tasks/README.md` | `Sdk="DotnetAffected.Tasks/…"` |

Sample command output is regenerated by running the tool, not edited. `Microsoft.Build.Traversal` appears in that
output and comes from the tool itself, so it changes when the tool changes it.

### At a major release

- The action major tracks the tool major, so `@vN` in the action pages moves with it. The tag has to exist before the
  docs claiming it are published.
- Add an upgrade guide for the new major under `upgrading/`, and a sidebar entry for it.
...
The docs describe the release, so they merge last:

1. Publish the tool to NuGet.
2. The action's smoke test goes green against it, then tag the action's new major.
3. Merge the docs PR, which publishes the site.
