...
namespace DotnetAffected.Core.Processor
{
    ...
    internal class AffectedProcessor : AffectedProcessorBase
    {
        ...
        private IEnumerable<ProjectGraphNode> DetermineAffectedProjects(ProjectGraphNode[] changedProjects,
        ...
        /// <returns></returns>
        private IEnumerable<ProjectGraphNode> DiscoverProjectsWithExclusivePackageChanges(
            AffectedProcessorContext context)
        {
            var projectsWithChangedPackages =
                (Dictionary<ProjectGraphNode, IEnumerable<PackageChange>>)context.Data[NugetDataCacheKey];
            return projectsWithChangedPackages
                .Where(kvp => kvp.Value.Any(p => kvp.Key.ReferencesNuGetPackage(p.Name)))
                .Select(kvp => kvp.Key);
        }

        private Dictionary<ProjectGraphNode, IEnumerable<PackageChange>> FindChangedNugetPackages(
            AffectedProcessorContext context)
        {
            ...
            if (context.RepositoryPath == changedPackageFiles[0]
                    .DirectoryName)
                changedPackageFiles = new List<FileInfo>
                {
            ...
            for (var i = 1; i < changedPackageFiles.Count; i++)
            {
                for (var q = 0; q < i; q++)
                {
                    if (changedPackageFiles[i]
                        .FullName.StartsWith(changedPackageFiles[q]
                            .DirectoryName!))
                    {
